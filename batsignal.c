...
int main(int argc, char *argv[])
{
	...
	// Read an integer value, this is the signal number.
	if (argc < 2) {
		batnum = 0;
	} else {
		batnum = atoi(argv[1]);
		if ((batnum < 1) || (batnum > MAX_BATNUM-1)) {
			printf("illegal signal number.\n");
			exit(-2);
		} else {
			printf("displaying signal %d\n", batnum);
		}
	}
	...
	// Display the batsignal
	for (face = 0; face < 4; face++) {
		thingy = batsignals[batnum];
		r = thingy[face*3 + 1];
		g = thingy[face*3 + 0];
		b = thingy[face*3 + 2];

		//printf("face %d r %d g %d b %d\n", face, r, g, b);
	...
	// Clear the inner LEDs
	for (face = 0; face < 4; face++) {
		//printf("face %d r %d g %d b %d\n", face, r, g, b);
		for (row = 0; row < ROWS_PER_FACE; row++) {
			set_inner_pixel(face, row, 0x80, 0x80, 0x80);
		}
	}	
